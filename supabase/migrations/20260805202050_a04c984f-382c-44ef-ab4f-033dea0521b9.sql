...
UPDATE public.payments SET idempotency_key = id::text WHERE idempotency_key IS NULL;
ALTER TABLE public.payments DROP CONSTRAINT IF EXISTS payments_idempotency_key_unique;
...
BEGIN
  ...
  -- Calculate remaining balance components from allocations
  SELECT 
