...
-- 1. ADICIONAR UNIQUE EM WHATSAPP PARA EMPLOYEES (Necessário para ON CONFLICT)
ALTER TABLE public.employees ADD CONSTRAINT employees_whatsapp_key UNIQUE (whatsapp);

-- 2. FUNCIONÁRIOS REAIS
...
-- 3. ARQUIVAR FUNCIONÁRIOS FICTÍCIOS
...
-- 4. EXPANDIR CADASTRO DE CLIENTE
...
-- 5. CONFIGURAÇÃO DE JUROS E MULTA NO EMPRÉSTIMO (SNAPSHOT)
...
ADD COLUMN IF NOT EXISTS delay_interest_rate_snapshot NUMERIC(5,4),
ADD COLUMN IF NOT EXISTS delay_interest_kind_snapshot TEXT DEFAULT 'diario';
...
-- 7. STORAGE POLICIES PARA BUCKET 'documents'
-- A função app.is_active_owner() não recebe argumentos (usa auth.uid() interno).
CREATE POLICY "Owners can access documents"
ON storage.objects
FOR ALL
TO authenticated
USING (bucket_id = 'documents' AND app.is_active_owner())
WITH CHECK (bucket_id = 'documents' AND app.is_active_owner());
